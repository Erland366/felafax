...
from . import xla_metadata as xla_metadata_lib
...
dtypes.ExtendedDType = ExtendedDType
...
def concretization_function_error(fun, suggest_astype=False):
  ...
  if fun is bool:
    ...
    #   raise TracerBoolConversionError(arg)
        raise TypeError(f"Value {arg!r} with type {type(arg)} is not a valid JAX type")
